...
export const googlePlacesFetchRequest = location => dispatch => {
  ...
  return superagent.get(`https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${location.lat},${location.lng}&radius=1600&type=restaurant&key=${process.env.__GAPI_KEY__}`)
    .then(res => {
        console.log('res: ', res);
